...
dim1=data_instance['dim1']
edges_1=eval(data_instance['edges_1'])
dim2=data_instance['dim2']
edges_2=eval(data_instance['edges_2'])
...
edges = []
for e in edges_1:
    edges.append((nodes[e[0][0]],nodes[e[0][1]],{'w':e[1]}))
...
def subplt(grafo_1, grafo_2,dim1,dim2):
    fig = plt.figure()
    plt.rcParams["figure.figsize"] = (15,7)
    ...
    labels = dict( ((i, j), i * dim1 + j) for i, j in grafo_1.nodes() )
    ...
    labels = dict( ((i, j), i * dim2 + j) for i, j in grafo_2.nodes() )
...
def visualizza_griglia(grafo,dim):
    pos = dict( (n, n) for n in grafo.nodes() )
    labels = dict( ((i, j), i * dim + j) for i, j in grafo.nodes() )
...
def verifica_ciclo_negativo(grafo,m,node_list,pt_green,pt_red, index_pt, return_only_boolean=False):
    n=len(node_list)
    aux=[]
    for node in node_list:
        aux.append((node//m, node%m))
    check = 0
    if return_only_boolean==False:
        visualizza_griglia(grafo,m)
    try:
        for i in range(1,n):
...
def verifica_presenza_solo_cicli_positivi(grafo,dim, pesi_nodi,pt_green,pt_red, index_pt, return_only_boolean=False):
    ...
    for (u,v) in G.edges:
        G[u][v]['w'] = G[u][v]['w'] - pesi_nodi[v[0]*dim+v[1]] + pesi_nodi[u[0]*dim+u[1]]
    if return_only_boolean==False:
        visualizza_griglia(G,dim)
    for (u,v) in G.edges:
        if G[u][v]['w'] < 0:
            if return_only_boolean:
                return False
...
subplt(griglia1,griglia2,dim1,dim2)
"""
cell_metadata={"hide_input": True, "editable": False,  "deletable": False, "tags": ["runcell"], "trusted": True}
add_cell(cell_type,cell_string,cell_metadata)
...
Forniscimi una lista di vertici che formano un ciclo negativo (rispetta la direzione degli archi).
        ...
        verif=f"""
if grafo_conciclo == 1:
    display(Markdown(verifica_ciclo_negativo(griglia1,dim1,ciclo_negativo,pt_green=1, pt_red={tasks[i]['tot_points']},index_pt={num_of_question - 1})))
if grafo_conciclo == 2:
    display(Markdown(verifica_ciclo_negativo(griglia2,dim2,ciclo_negativo,pt_green=1, pt_red={tasks[i]['tot_points']},index_pt={num_of_question - 1})))"""
...
if grafo_senzaciclo == 1:
    display(Markdown(verifica_presenza_solo_cicli_positivi(griglia1,dim1,pesi_nodi,pt_green=1, pt_red={tasks[i]['tot_points']},index_pt={num_of_question - 1})))
if grafo_senzaciclo == 2:
    display(Markdown(verifica_presenza_solo_cicli_positivi(griglia2,dim2,pesi_nodi,pt_green=1, pt_red={tasks[i]['tot_points']},index_pt={num_of_question - 1})))"""
    else:
        assert False
    # ( CELL request:
