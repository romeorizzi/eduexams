...
def verifica(raggi,pt_green,pt_red, index_pt, silent=False):
    ...
    if conta_num_met_in(quadro_scratch)==0:
        if silent:
            ...
            display(Markdown(evaluation_format("Si", pt_green, pt_red,index_pt)+"Non posso dirti se hai usato il minimo numero di raggi ma la buona notizia è che hai distrutto tutti gli asteroidi. L'Enterprise NCC-1701 Vulcan è salva. L'equipaggio ringrazia il capitano."))
    else:
        if silent:
            return False
        else:
            display(Markdown(evaluation_format("No", 0, pt_red,index_pt)+f"Non hai colpito tutti gli asteroidi. Oh ... Kirk, controlla quelli che ti restano:"))
...
significa che spareresti con i cannoni delle righe$~3$,$5$ e$~6$ e con i cannoni delle colonne$~2$,$4$ e$~5$. Ricordati che gli indici di riga e colonna partono da$~0$.
