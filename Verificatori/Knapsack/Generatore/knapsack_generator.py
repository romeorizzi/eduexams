...
dictionary_of_types = {
    ...
    "R3": f"Trovare la somma massima dei valori degli elementi  <b> se non si considerano i seguenti elementi {tasks[2]['edr']} </b>.",
    "R4": f"Trovare la somma massima dei valori degli elementi <b> se __B__ diventa {tasks[3]['B2']}</b>.",
}
...
for i in range (0,len(tasks)):
    request=f"{num_of_question}. __[{tasks[i]['tot_points']} pts]__ {dictionary_of_types[tasks[i]['request']]}"
    
    if tasks[i]['request'] == "R3":
        verif=f"display(Markdown(verifknapsack(elementi,pesi,valori,B,answer{num_of_question - 1}, pt_green=1, pt_red={tasks[2]['tot_points']},index_pt={num_of_question-1},edr={tasks[2]['edr']})))"
    elif tasks[i]['request'] == "R4":
        verif=f"display(Markdown(verifknapsack(elementi,pesi,valori,{tasks[3]['B2']},answer{num_of_question - 1}, pt_green=1, pt_red={tasks[3]['tot_points']},index_pt={num_of_question-1})))"
    else:
